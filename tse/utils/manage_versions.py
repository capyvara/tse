...
def getargs():
    parser = argparse.ArgumentParser(description="Manages '.ver' version files directories")
    subparsers = parser.add_subparsers(help="Command", dest="command")
    pack = subparsers.add_parser("pack", help="Packs the files inside '.ver' directories in a single zip per dir")
    pack.add_argument("path", type=dir_path)
...
def pack(root, files):
    ...
    backup_path = os.path.join(root, ".bpk_pack.zip")
...
print(f"Scanning '.ver' directories under {args.path}...")
print("")

for root, dirs, files in os.walk(args.path):
    if os.path.basename(root) != ".ver":
        continue
